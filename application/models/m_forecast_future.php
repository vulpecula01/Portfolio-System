...
class M_forecast_future extends Port_model {

	function generate_pos_1()//ข้าราชการระดับปฏิบัติงาน
{
	// $usr_id = 34;
	...
	$port_user = $this->gets_join(); //เรียกไอดี user
	...
	foreach($port_user as $row){
	
		if($row['pos_id'] == 1){//เช็คตำแหน่ง
			if($row['dec_id'] >= 1){//เช็คเครื่องอะไรแล้ว
				...
				if($row['dec_id'] >= 2){
					...
					if($row['dec_id'] >= 3){
						...
						if($row['dec_id'] >= 4){
							...
							if($row['usr_salary'] >= 10190 && $date - $row['usr_dateforwork'] >= 10){
...
function generate_pos_19()//ผู้ช่วยศาสตราจารย์หรืออาจารย์
{
	
	 //$ports_user = $this->gets();
	//$date = ( date('Y')+543 );
	//return $ports_user;
	...
	foreach($ports_user as $port_user){

		return $port_user;
	}
	
	// 		// if($row['pos_id'] == 19){//เช็คตำแหน่ง
	// 		// 	if( (int) $row['dec_id'] >= 4){//เช็คเครื่องอะไรแล้ว
	...
	// 		// 								return 7;
	// 		// 							}else {
	// 		// 								if(( date('Y')+543 )-$row['usr_dateforwork'] >= 20){
									
	// 		// 								return 7;
	// 		// 								}
	// 		// 							}
	// 		// 					}else {
	// 		// 						if(( date('Y')+543 )-$row['usr_dateforwork'] >= 15){
	...
	// 		// 				if(( date('Y')+543 )-$row['usr_dateforwork'] >= 10){
								
	// 		// 					return 5;
	// 		// 				} 
	// 		// 			}
	// 		// 	}else{
	// 		// 		if(( date('Y')+543 )-$row['usr_dateforwork'] >= 5){
...
function generate_pos_26($usr_id)//ลูกจ้างประจำระดับ 2
{
	...
	public function gets_join(){
		$this->db->distinct();
		$this->db->select('*');
		$this->db->from('port_user');
		$this->db->join('port_insignia', 'port_insignia.usr_id = port_user.usr_id');
		$this->db->order_by('dec_id', "DESC");
